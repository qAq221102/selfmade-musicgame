...
def load_video():
    global fps, Video_1st_image, video_total_frame, prev_frame, Selected_Cap
    if Music_Play:
        # 取得影片的寬度和高度
...
def get_1st_img():
    global Selected_Cap, Video_1st_image
    ...
    while True:
        ...
        ret, frame = Selected_Cap.read()
        img = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        img = pygame.image.frombuffer(img.tobytes(), (video_width, video_height), "RGB")
        new_image = pygame.transform.scale(img, (int(Width), int(Height)))
        Video_1st_image = new_image
        Video_1st_image.set_alpha(200)
        screen.blit(new_image, (0, 0))
...
def for_render_thread():
    while True:
        s = pygame.time.get_ticks()
        if not (Gaming_Exit_Flag or Select_Flag):
            screen.fill((0, 0, 0))
            load_video()
            draw_game_region()
...
def open_thread(switch):
    if switch:
        ...
        render_thread = threading.Thread(target=for_render_thread)
...
while True:
    ...
    # 開啟多線程
    open_thread(switch)
    video_target_ms = MusicPlayer.get_pos()
    # 背景影片
    # 滑鼠區域
    # 顯示角落圓形
    if pressed_combo:
        print(f"pos: {str(pygame.mouse.get_pos())}")
        textprint(
            file_path_dict[gaming_font_filename],
            Height // 16,
